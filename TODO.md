...
- [ ] 邀请注册功能，增强站点运营
- [ ] 微信小程序 【预计4.0版本】
- [ ] 控制面板echarts统计图表 —— 统计每天、每周、每月各种数据增长曲线图，如文件量、用户注册量、每天签到量等
...
- [ ] 积分商城
- [ ] 评论管理；用户可以在前台删除自己的评论；删除评论之后，文件评分发生变化
- [ ] 程序以微服务形式做成模块化，拆分成：web模块、爬虫模块、文件转换模块、全文搜索模块、用户模块、云存储模块
- [ ] 管理后台，管控是否开放注册控制
- [ ] url路由链接调整？
- [ ] 凌晨自动更新站点地图
- [ ] 文库频道，可在管理后台更新频道图片封面
- [ ] 文件分类数据统计修正(管理后台处理)
- [ ] 频道图片上传和更换功能
- [ ] 管理后台增加一个上传文件必须达到的最低积分要求，避免一些新用户上传垃圾文件刷积分
...
- [x] 多样化存储实现
    ...
    - [ ] UCloud
    - [ ] 青云
- [x] SEO优化，参考新浪爱问文库，从文件中提取文本，放到HTML页面隐藏显示的div中
- [x] 积分问题[#40](https://github.com/TruthHun/DocHub/issues/40)
- [x] 封面、横幅裁剪，不再依赖云存储做图片处理
- [x] `sudo`支持
- [x] 下载文件出错，下载id为100的文件，可能下载到id位80的文件（MySQL查询语句错误造成的）
- [x] 管理后台测试的时候，提示先保存
...
- [x] 文件评分人数统计问题(评分出现-1的情况)
- [x] 文件分享人可把文件设置为不可下载状态
- [x] 邮箱配置更改的时候进行验证
...
文件上传流程优化：
    1. 未登录用户，不允许上传文件
    2. 已登录用户，积分达不到管理后台规定的积分数，不允许上传文件
    3. 用户上传文件的时候，前端计算文件 MD5 的值，然后请求后台对比值是否存在，存在了，则表示文件已经存在，直接`秒传`
    4. 用户上传的文件在管理后台不存在，则把原文件上传到云存储，文件信息(分类、大小、md5等)、用户积分变化等存入数据库
    5. 数据入库成功之后，再进行文件转换，文件最终转换成功之后，更新文件状态
    
    (文件处理，分已存在和未存在的文件进行处理)
...
- [x] 修复搜索的时候，MySQL like 查询，文件在排序的时候查询不到数据的问题
- [x] 文件图标优化 
- [x] PC端个人中心页面大调整
- [x] 程序后端代码优化
- [x] 使用`govender`管理依赖包，方便需要做二次开发的朋友
- [x] epub、mobi等文件转PDF参数优化
...
- [x] 文件阅读页面再优化，修复v1.1版本优化造成的Bug——剩余未阅读页数不准确、无法正确翻页等问题。
...
- [x] 单页优化
- [x] 批量更新索引
- [x] 文件新建和更新时的TimeUpdate，即更新时间
- [x] 删除文件或者恢复文件的时候删除或者更新索引
...
- [x] 用户头像和文件封面等默认图片优化，在加载图片的时候直接在前端使用`onerror`，不再在后端查询oss中图片是否存在以及不存在时返回默认图片。
- [x] 所有相关配置项，为了配置的方便，都统一放到app.conf文件。配置文件中的每一项，都加上了详尽的配置说明。
- [x] mobi、epub、chm、txt等格式文件在线浏览的实现支持。
- [x] 解决邮件发送问题，统一使用SMTP发送邮件，并实现对TLS/SSL邮件的发送支持。使用了https://github.com/go-gomail/gomail库。
- [x] 文件阅读页面性能优化
- [x] 文件预览页数限制(可在`管理后台`->`系统设置`->`文件最大预览页数`做限制。这样的好处就是，如果一个300页的文件，只提供100页给用户阅读，可以减少服务器后端PDF转svg的资源开销，也可以促进用户使用积分下载文件...新浪爱问共享资料就是这么干的...)