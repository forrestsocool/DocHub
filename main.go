...
func init() {
	...
	beego.AddFuncMap("SimpleList", models.NewDocument().TplSimpleList)  //简易的文件列表
	beego.AddFuncMap("HandlePageNum", helper.HandlePageNum)             //处理文件页码为0的显示问题
	beego.AddFuncMap("DoesCollect", models.DoesCollect)                 //判断用户是否已收藏了该文件
	...
	beego.AddFuncMap("IsIllegal", models.NewDocument().IsIllegal)       //根据md5判断文件是否是非法文件
	beego.AddFuncMap("IsRemark", models.NewDocumentRemark().IsRemark)   //根据文件是否存在备注
