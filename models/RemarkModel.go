...
//文件备注，用于侵权文件等的部分内容的预览展示，并在文件预览页面挂上跳转购买正版的导购链接；同时对于一些开源书籍，也可以一面提供站内文件的下载，一面引导用户购买正版。
type DocumentRemark struct {
	Id            int    `orm:"column(Id)"`                           //document_store表中的ID，当前台页面查询文件的备注的时候，以DsId作为document_remark表中的主键进行查询
	Content       string `orm:"column(Content);size(5120);default()"` //备注内容内容
	AllowDownload bool   `orm:"column(AllowDownload);default(true)"`  //是否允许下载文件
