...
type DocText struct {
	...
	Md5     string `orm:"size(32);default();column(Md5)"`       //文件的md5，之所以存储md5而不是文件的存档id，主要是因为文件在这之前没录入数据库
	Content string `orm:"size(5000);default();column(Content)"` //文件提取到的文件内容
}
